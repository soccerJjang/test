...
const ProVideo = (props) => {
    const videoUri = props.route.params.video;
    ...
    React.useEffect(() => {
        ...
        let rec = '';
        (async () => {
            await FileSystem.downloadAsync(`http://223.130.132.180:5009/upload/${videoUri.video}`, FileSystem.cacheDirectory + "video.mp4")
            .then(data => {
                rec = data.uri;
